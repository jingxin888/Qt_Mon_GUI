...
void HalLcd_HW_Clear(void)
{
   ...
   //屏蔽的SET_DDRAM_ADDR(0x00);
   //屏蔽的or (n = 0; n < (LCD_MAX_LINE_COUNT * HAL_LCD_MAX_CHARS); n++)
   //屏蔽的{
     //我屏蔽的HalLcd_HW_Write(' ');
   //屏蔽的}
...
void HalLcd_HW_Init(void)
{
  ...
  /* Perform reset */
  //屏蔽的LCD_ACTIVATE_RESET();
  //屏蔽的HalLcd_HW_WaitUs(15000); // 15 ms
  //屏蔽的LCD_RELEASE_RESET();
  //屏蔽的HalLcd_HW_WaitUs(15); // 15 us

  /* Perform the initialization sequence */
  //屏蔽的FUNCTION_SET(CGRAM | COM_FORWARD | THREE_LINE);

  /* Set contrast */
  //屏蔽的HalLcd_HW_SetContrast(15);

  /* Set power */
  //屏蔽的SET_POWER_SAVE_MODE(OSC_OFF | POWER_SAVE_ON);
  //屏蔽的SET_POWER_CTRL(VOLTAGE_DIVIDER_ON | CONVERTER_AND_REG_ON);
  //屏蔽的SET_BIAS_CTRL(BIAS_1_5);
  //屏蔽的HalLcd_HW_WaitUs(21000);// 21 ms

  /* Clear the display */
  //屏蔽的HalLcd_HW_Clear();
  //屏蔽的HalLcd_HW_ClearAllSpecChars();
  //屏蔽的SET_DISPLAY_CTRL(DISPLAY_CTRL_ON | DISPLAY_CTRL_BLINK_OFF | DISPLAY_CTRL_CURSOR_OFF);
//以下是我加的
...
void HalLcd_HW_WriteLine(uint8 line, const char *pText)
{
  //把这个函数内容屏蔽掉
  //屏蔽的uint8 count;
  //屏蔽的uint8 totalLength = (uint8)osal_strlen( (char *)pText );

  /* Write the content first */
  //屏蔽的for (count=0; count<totalLength; count++)
  //屏蔽的{
    //屏蔽的HalLcd_HW_WriteChar(line, count, (*(pText++)));
  //屏蔽的}

  /* Write blank spaces to rest of the line */
  //屏蔽的for(count=totalLength; count<HAL_LCD_MAX_CHARS;count++)
  //屏蔽的{
  //屏蔽的  HalLcd_HW_WriteChar(line, count, ' ');
  //屏蔽的}
